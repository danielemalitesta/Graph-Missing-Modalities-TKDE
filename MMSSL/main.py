...
class Trainer(object):
    def __init__(self, data_config):
        ...
        if args.method == 'feat_prop':
            imputed_visual = f'{root}/visual_embeddings_{args.method}_{args.missing_layers}_{args.top_k}_indexed'
            imputed_textual = f'{root}/textual_embeddings_{args.method}_{args.missing_layers}_{args.top_k}_indexed'
            for it in missing_visual:
                visual_embeddings[it] = np.load(f'{imputed_visual}/{it}.npy')
            for it in missing_textual:
                textual_embeddings[it] = np.load(f'{imputed_textual}/{it}.npy')
        ...
        if args.method == 'heat':
            imputed_visual = f'{root}/visual_embeddings_{args.method}_{args.missing_layers}_{args.top_k}_{args.time}_indexed'
            imputed_textual = f'{root}/textual_embeddings_{args.method}_{args.missing_layers}_{args.top_k}_{args.time}_indexed'
            for it in missing_visual:
                visual_embeddings[it] = np.load(f'{imputed_visual}/{it}.npy')
            for it in missing_textual:
                textual_embeddings[it] = np.load(f'{imputed_textual}/{it}.npy')
        ...
        if args.method == 'pers_page_rank':
            imputed_visual = f'{root}/visual_embeddings_{args.method}_{args.missing_layers}_{args.top_k}_{args.alpha}_indexed'
            imputed_textual = f'{root}/textual_embeddings_{args.method}_{args.missing_layers}_{args.top_k}_{args.alpha}_indexed'
            for it in missing_visual:
                visual_embeddings[it] = np.load(f'{imputed_visual}/{it}.npy')
            for it in missing_textual:
                textual_embeddings[it] = np.load(f'{imputed_textual}/{it}.npy')
        ...
        self.image_feats = visual_embeddings
        self.text_feats = textual_embeddings

        self.image_feat_dim = self.image_feats.shape[-1]
        self.text_feat_dim = self.text_feats.shape[-1]
        self.ui_graph = self.ui_graph_raw = pickle.load(open(args.data_path + args.dataset + '/trnMat.pkl','rb')).tocsr()
        self.image_ui_graph_tmp = self.text_ui_graph_tmp = torch.tensor(self.ui_graph_raw.todense()).to(self.device)
        self.image_iu_graph_tmp = self.text_iu_graph_tmp = torch.tensor(self.ui_graph_raw.T.todense()).to(self.device)
    ...
    def u_sim_calculation(self, users, user_final, item_final):
        ...
        u_ui = torch.tensor(self.ui_graph_raw[users].todense()).to(self.device)
    ...
    def train(self):
        ...
        for epoch in range(args.epoch):
            ...
            for idx in tqdm(range(n_batch)):
                ...
                lossf = (predf.mean())
                u_ui = torch.tensor(self.ui_graph_raw[users].todense()).to(self.device)
                u_ui = F.softmax(u_ui - args.log_log_scale*torch.log(-torch.log(torch.empty((u_ui.shape[0], u_ui.shape[1]), dtype=torch.float32).uniform_(0,1).to(self.device)+1e-8)+1e-8)/args.real_data_tau, dim=1) #0.002  
        ...
        print(f'Best validation: {round(best_recall, 4)}')
        print(f'Test results --> recall@20: {round(test_ret["recall"][1], 4)}\tndcg@20: {round(test_ret["ndcg"][1], 4)}')
...
if __name__ == '__main__':
    ...
    hyperparams = ParameterGrid({
        "--D_lr": [2.5e-4, 3e-4, 3.5e-4],
        "--lr": [5e-4, 5.4e-3, 5.6e-3]
    })
    ...
    for hyp in hyperparams:
        ...
        print('END OF THE CURRENT HYPERPARAMETER SEARCH')
