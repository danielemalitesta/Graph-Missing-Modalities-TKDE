...
if args.method == 'heat':
    ...
    if args.model == 'vbpr':
      ...
      data_config:
        ...
        side_information:
          - dataloader: VisualAttribute
            visual_features: ../data/{args.data}/visual_embeddings_gae_{args.top_k}_complete_indexed
          - dataloader: TextualAttribute
            textual_features: ../data/{args.data}/textual_embeddings_gae_{args.top_k}_complete_indexed
      dataset: dataset_name
      top_k: 20
      evaluation:
        cutoffs: [20]
    ...
    with open(f'./config_files/{args.model}_gae_{args.top_k}_{args.data}.yml', 'w') as f:
        f.write(config.format(args.data).replace('dataset_name', args.data))

    visual_folder_imputed_indexed = f'./data/{args.data}/visual_embeddings_gae_{args.top_k}_indexed'
    textual_folder_imputed_indexed = f'./data/{args.data}/textual_embeddings_gae_{args.top_k}_indexed'
    visual_folder_complete = f'./data/{args.data}/visual_embeddings_gae_{args.top_k}_complete_indexed'
    textual_folder_complete = f'./data/{args.data}/textual_embeddings_gae_{args.top_k}_complete_indexed'
    ...
    run_experiment(f"config_files/{args.model}_gae_{args.top_k}_{args.data}.yml")

    shutil.rmtree(visual_folder_complete)
    shutil.rmtree(textual_folder_complete)

    os.remove(f"config_files/{args.model}_gae_{args.top_k}_{args.data}.yml")
