...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def _valid_epoch(self, valid_data, is_test=False, idx=0, extractor='RNet50+SBert'):
        ...
        Returns:
            ...
            dict: valid result
        """
        x_df, file_path = None, None
        if is_test:
            valid_result, x_df, file_path = self.evaluate(valid_data, is_test=is_test, idx=idx, extractor=extractor)
        else:
            valid_result, _, _ = self.evaluate(valid_data, is_test=is_test, idx=idx, extractor=extractor)
        valid_score = valid_result[self.valid_metric] if self.valid_metric else valid_result['NDCG@20']
        return valid_score, valid_result, x_df, file_path
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True, idx=0, extractor='RNet50+SBert'):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_score, valid_result, _, _ = self._valid_epoch(valid_data)
                ...
                # test
                _, test_result, x_df, file_path = self._valid_epoch(test_data, is_test=True, idx=idx, extractor=extractor)
                if verbose:
                ...
                if stop_flag:
                    ...
                    break
        return self.best_valid_score, self.best_valid_result, self.best_test_upon_valid, self.best_x_df, self.best_file_path


    @torch.no_grad()
    def evaluate(self, eval_data, is_test=False, idx=0, extractor='RNet50+SBert'):
        r"""Evaluate the model based on the eval data.
        Returns:
        ...
        return self.evaluator.evaluate(batch_matrix_list, eval_data, is_test=is_test, idx=idx, extractor=extractor)
