...
topk_metrics = {metric.lower(): metric for metric in ['Recall', 'Recall2', 'HR', 'Precision', 'NDCG', 'MAP']}
...
class TopKEvaluator(object):
    ...
    def evaluate(self, batch_matrix_list, eval_data, is_test=False, idx=0, extractor='RNet50+SBert'):
        ...
        if self.save_recom_result and is_test:
            ...
            file_path = os.path.join(dir_name, '{}-{}-top{}-idx{}-extractor{}.csv'.format(
                model_name, dataset_name, max_k, idx, extractor))
            x_df = pd.DataFrame(topk_index)
            x_df.insert(0, 'user', eval_data.get_eval_users())
            x_df = x_df.melt(id_vars='user', value_name='item')
            x_df = x_df[['user', 'item']]
            x_df = x_df.sort_values(['user']).reset_index(drop=True)
        ...
        return metric_dict, x_df, file_path
