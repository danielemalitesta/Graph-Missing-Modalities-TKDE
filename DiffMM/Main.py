...
class Coach:
	...
	def run(self):
		...
		for ep in range(0, args.epoch):
			...
			if tstFlag:
				val_reses = self.testEpoch()
				test_reses = self.testEpoch(val=False)
				if (val_reses['Recall'] > val_recallMax):
					val_recallMax = val_reses['Recall']
					recallMax = test_reses['Recall']
					ndcgMax = test_reses['NDCG']
					precisionMax = test_reses['Precision']
					bestEpoch = ep
				log(self.makePrint('Val', ep, val_reses, tstFlag))
				log(self.makePrint('Test', ep, test_reses, tstFlag))
			print()
		print('Best validation Recall: %.4f' % val_recallMax)
		print('Best epoch: ', bestEpoch, ' , Recall : ', round(recallMax, 4), ' , NDCG : ', round(ndcgMax, 4), ' , Precision', round(precisionMax, 4))

	def prepareModel(self):
		if args.data == 'tiktok':
			self.model = Model(self.handler.image_feats.detach(), self.handler.text_feats.detach(), self.handler.audio_feats.detach()).cuda()
		else:
			self.model = Model(self.handler.image_feats.detach(), self.handler.text_feats.detach()).to(self.device)
		...
		in_dims = out_dims[::-1]
		self.denoise_model_image = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm).to(self.device)
		self.denoise_opt_image = torch.optim.Adam(self.denoise_model_image.parameters(), lr=args.lr, weight_decay=0)

		out_dims = eval(args.dims) + [args.item]
		in_dims = out_dims[::-1]
		self.denoise_model_text = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm).to(self.device)
		self.denoise_opt_text = torch.optim.Adam(self.denoise_model_text.parameters(), lr=args.lr, weight_decay=0)

		if args.data == 'tiktok':
			out_dims = eval(args.dims) + [args.item]
			in_dims = out_dims[::-1]
			self.denoise_model_audio = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm).to(self.device)
	...
	def testEpoch(self, val=True):
		if val:
			tstLoader = self.handler.valLoader
		else:
		...
		for usr, trnMask in tstLoader:
			...
			recall, ndcg, precision = self.calcRes(topLocs.cpu().numpy(), tstLoader.dataset.tstLocs, usr)
			epRecall += recall
			epNdcg += ndcg
...
if __name__ == '__main__':
	...
	if args.method in ['gae', 'neigh_mean']:
		...
		imputation = f'{args.method}_{args.top_k}'
	...
	if args.method == 'feat_prop':
		...
		imputation = f'{args.method}_{args.layers}_{args.top_k}'
	...
	if args.method == 'heat':
		...
		imputation = f'{args.method}_{args.layers}_{args.top_k}_{args.temp}'
	...
	if args.method == 'pers_page_rank':
		...
		imputation = f'{args.method}_{args.layers}_{args.top_k}_{args.alpha}'
	...
	for hyp in hyperparams:
		args.ssl_reg = hyp['--ssl_reg']
		args.reg = hyp['--reg']
		args.e_loss = hyp['--e_loss']
		args.trans = hyp['--trans']
		seed_it(args.seed)
		...
		log('Start')
		handler = DataHandler()
		handler.LoadData(visual_embeddings, textual_embeddings)
		log('Load Data')
